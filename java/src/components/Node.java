...
public class Node {
    ...
    public Node(int cpu, int mem, int netIn, int netOut,Solver solver,Graph graph) {
        int nbComponent = graph.getComponents().size();
    ...
    private void coherenceconstraints(){
        ...
        solver.post(SetConstraintsFactory.sum(this.getCompSurNoeud(),this.graph.getComponentsRequiredCpu()
                ,0,this.getCpuConso(),true));

        // idem avec la mémoire
        solver.post(SetConstraintsFactory.sum(this.getCompSurNoeud(),this.getGraph().getComponentsRequiredmem()
